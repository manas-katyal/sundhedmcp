...
import { connectPage, connectSignInPage, loginPage, returningPage, signedInPage, signInFailedPage, statusPage } from "./pages.ts";
...
export function createApp() {
  ...
  // Changing ADMIN_PASSWORD logs every client out.
  const fingerprint = createHash("sha256").update(config.adminPassword).digest("hex");
  ...
  app.get("/", async (_req, res) => {
    ...
    res.type("html").send(statusPage({ problems: setupProblems(), mcpUrl: mcpUrl.href, loggedIn: s.loggedIn }));
  });
  ...
  app.get("/connect", async (req, res) => {
    if (setupProblems().length) return void res.redirect(303, "/");
    if (!viewerOk(req)) return void res.type("html").send(connectSignInPage());
  ...
  app.post("/connect/signin", express.urlencoded({ extended: false, limit: "4kb" }), (req, res) => {
    ...
    if (f && f.until > Date.now()) return void res.status(429).type("html").send(connectSignInPage("Too many attempts. Try again in 15 minutes."));
    if (!verifyPassword(String((req.body as Record<string, string>).password ?? ""))) {
      ...
      return void res.status(401).type("html").send(connectSignInPage("Wrong password."));
    }
    failures.delete(ip);
