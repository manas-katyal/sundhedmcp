...
export class SingleUserProvider implements OAuthServerProvider {
  ...
  get clientsStore(): OAuthRegisteredClientsStore {
    ...
    return {
      getClient(clientId) {
        return store.data.clients[clientId] as OAuthClientInformationFull | undefined;
      },
      registerClient(client) {
        ...
        store.update((d) => {
          ...
          const ids = Object.keys(d.clients);
          if (ids.length > 20) for (const id of ids.slice(0, ids.length - 20)) delete d.clients[id];
          d.clients[full.client_id] = full;
        });
  ...
  completeLogin(requestId: string, password: string, ip: string): { redirect: string } | { done: true } | { error: string; requestId?: string } {
    ...
    const code = token();
    this.store.update((d) => {
      for (const [c, v] of Object.entries(d.codes)) if (v.expires < now()) delete d.codes[c];
      d.codes[sha256(code)] = {
  ...
  async challengeForAuthorizationCode(client: OAuthClientInformationFull, authorizationCode: string): Promise<string> {
    const c = this.store.data.codes[sha256(authorizationCode)];
  ...
  async exchangeAuthorizationCode(client: OAuthClientInformationFull, authorizationCode: string, _codeVerifier?: string, redirectUri?: string, resource?: URL): Promise<OAuthTokens> {
    ...
    const c = this.store.data.codes[key];
    const fail = (why: string) => {
      console.log(`[sundhed ${new Date().toISOString()}] token exchange refused for ${client.client_name ?? client.client_id}: ${why}`);
    ...
    return this.store.update((d) => {
      delete d.codes[key];
      return this.issue(d.tokens, client.client_id, c.scopes, c.resource);
    });
  }
  ...
  async exchangeRefreshToken(client: OAuthClientInformationFull, refreshToken: string, scopes?: string[], resource?: URL): Promise<OAuthTokens> {
    ...
    const t = this.store.data.tokens[key];
    ...
    return this.store.update((d) => {
      delete d.tokens[key];
      return this.issue(d.tokens, client.client_id, scopes?.length ? scopes : t.scopes, t.resource);
    });
  }

  async verifyAccessToken(tokenValue: string): Promise<AuthInfo> {
    const t = this.store.data.tokens[sha256(tokenValue)];
  ...
  async revokeToken(client: OAuthClientInformationFull, request: OAuthTokenRevocationRequest): Promise<void> {
    ...
    const t = this.store.data.tokens[key];
    if (t && t.client_id === client.client_id) this.store.update((d) => void delete d.tokens[key]);
  }
